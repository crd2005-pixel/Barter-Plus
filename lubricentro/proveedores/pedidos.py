...
from PyQt5.QtWidgets import (
    ...
    QTableWidgetItem, QMessageBox, QComboBox, QLabel, QFormLayout
)
...
class PedidosTab(QWidget):
    def __init__(self, parent=None):
        ...
        self.cmb_marca.addItem("Todas las Marcas", "")
        self.cmb_rubro.addItem("Todos los Rubros", "")
        self.cmb_subrubro.addItem("Todos los Subrubros", "")

        self.cmb_marca.currentIndexChanged.connect(self._apply_filters)
        self.cmb_rubro.currentIndexChanged.connect(self._apply_filters)
        self.cmb_subrubro.currentIndexChanged.connect(self._apply_filters)
    ...
    def _populate_filters(self):
        ...
        self.cmb_subrubro.addItem("Todos los Subrubros", "")
        with SessionLocal() as s:
            # Dynamically fetch subrubros from db
            subrubros = s.query(Producto.subrubro).filter(Producto.subrubro.isnot(None), Producto.subrubro != "").distinct().order_by(Producto.subrubro).all()
            for (sub,) in subrubros:
                self.cmb_subrubro.addItem(sub, sub)
    ...
    def _fill_table(self, items_list):
        ...
        # Apply filters
        filtered_items = []
        sel_marca = self.cmb_marca.currentText() if self.cmb_marca.currentIndex() > 0 else ""
        sel_rubro = self.cmb_rubro.currentText() if self.cmb_rubro.currentIndex() > 0 else ""
        sel_subrubro = self.cmb_subrubro.currentText() if self.cmb_subrubro.currentIndex() > 0 else ""

        for it in items_list:
            # For filter checking we can assume `marca`, `rubro` exist
            if sel_marca and it.get("marca", "") != sel_marca:
                continue
            if sel_rubro and it.get("rubro", "") != sel_rubro:
                continue
            # Note: _build_item_dict might not have subrubro in dict, we will check it
            if sel_subrubro and it.get("subrubro", "") != sel_subrubro:
                continue
            filtered_items.append(it)

        # Sort by provider then product (agrupación lógica)
        sorted_items = sorted(filtered_items, key=lambda x: (x.get("proveedor",""), x.get("producto","")))
