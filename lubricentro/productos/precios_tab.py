...
class ExcepcionesGananciaDialog(QDialog):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle("Ganancia por Rubro / Subrubro")
        self.resize(500, 400)
    ...
    def _setup_ui(self):
        ...
        self.tbl.setHorizontalHeaderLabels(["Rubro", "Subrubro", "Ganancia (%)"])
        self.tbl.horizontalHeader().setSectionResizeMode(0, QHeaderView.Stretch)
        self.tbl.horizontalHeader().setSectionResizeMode(1, QHeaderView.Stretch)
        lay.addWidget(self.tbl)

        lbl = QLabel("Dejá en blanco (0.00) para usar la ganancia global/padre.\nPrioridad: Subrubro > Rubro > Global.")
    ...
    def _get_rubro_tree(self):
        return {
            "Filtros": ["Aceite", "Aire", "Combustible", "Habitaculo"],
            "Lubricantes": ["Mineral", "Semi-Sintetico", "Sintetico"],
            "Baterias": ["Auto", "Moto", "Camion"],
            "Otro": ["Lamparas", "Escobillas", "Varios"]
        }

    def _load_data(self):
        tree = self._get_rubro_tree()
        rows = []
        for r, subs in tree.items():
            rows.append((r, "", self._get_saved_val(r, "")))
            for s in subs:
                rows.append((r, s, self._get_saved_val(r, s)))

        self.tbl.setRowCount(len(rows))
        for i, (r, s, val) in enumerate(rows):
            it_r = QTableWidgetItem(r)
            it_r.setFlags(Qt.ItemIsEnabled)
            if not s:
                # Es un rubro principal
                font = QFont()
                font.setBold(True)
                it_r.setFont(font)
                it_s = QTableWidgetItem("(Todos)")
                it_s.setFlags(Qt.ItemIsEnabled)
                it_s.setFont(font)
            else:
                it_s = QTableWidgetItem(s)
                it_s.setFlags(Qt.ItemIsEnabled)

            self.tbl.setItem(i, 0, it_r)
            self.tbl.setItem(i, 1, it_s)

            sp = QDoubleSpinBox()
            sp.setRange(0.0, 500.0)
            sp.setDecimals(2)
            sp.setSingleStep(0.5)
            sp.setSpecialValueText(" ") # Si es 0, no mostrar nada
            sp.setValue(val)
            self.tbl.setCellWidget(i, 2, sp)

    def _get_saved_val(self, rubro, subrubro):
        key = f"productos/precios/ganancia_{rubro}_{subrubro}"
        try:
            val = self._settings.value(key, 0.0, type=float)
        except TypeError:
            val = float(self._settings.value(key, 0.0) or 0.0)
        return val

    def _save_and_accept(self):
        for i in range(self.tbl.rowCount()):
            r = self.tbl.item(i, 0).text()
            s = self.tbl.item(i, 1).text()
            if s == "(Todos)": s = ""
            sp = self.tbl.cellWidget(i, 2)
            key = f"productos/precios/ganancia_{r}_{s}"
            self._settings.setValue(key, sp.value())
        self.accept()

def get_ganancia_override(rubro, subrubro, settings):
    # Try specific subrubro
    key_sub = f"productos/precios/ganancia_{rubro}_{subrubro}"
    try:
        val_sub = settings.value(key_sub, 0.0, type=float)
    except TypeError:
        val_sub = float(settings.value(key_sub, 0.0) or 0.0)

    if val_sub > 0.001:
        return val_sub

    # Try general rubro
    key_rub = f"productos/precios/ganancia_{rubro}_"
    try:
        val_rub = settings.value(key_rub, 0.0, type=float)
    except TypeError:
        val_rub = float(settings.value(key_rub, 0.0) or 0.0)
...
class PreciosTab(QWidget):
    ...
    def _setup_ui(self):
        ...
        self.btn_excepciones = QPushButton("Excepciones por Rubro...")
        top.addWidget(self.btn_excepciones)
        ...
        self.btn_excepciones.clicked.connect(self._open_excepciones_dialog)
        ...
        try:
            ...
            for r in self._rows:
                if r["pk"] == pk:
                    ...
                    p_obj = r["_prod_obj"]
                    rubro = getattr(p_obj, "rubro", "") or ""
                    subrubro = getattr(p_obj, "subrubro", "") or ""

                    gan_pct = get_ganancia_override(rubro, subrubro, self._settings)
                    if gan_pct is None:
                        gan_pct = float(self.sp_gan.value() or 0.0)
    ...
    def _recalc_all(self):
        # Gastos Fijos (Disabled)
        self.lbl_fijos.setText("Gastos fijos: Desactivados")

        global_gan_pct = float(self.sp_gan.value() or 0.0)

        for row in self._rows:
            prorr = 0.0
            precio_cp = row["mas_iva"] # Costo + IVA
    ...
    # Exportar PDF
    # ------------------------------------------------------------------
    def _open_excepciones_dialog(self):
        dlg = ExcepcionesGananciaDialog(self)
