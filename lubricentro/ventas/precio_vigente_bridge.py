...
from productos.precios_tab import get_ganancia_override
...
def _precio_final_base(p: Producto, total_fijos: float, denom_total: float, ganancia_pct: float) -> float:
    ...
    gan_pct = get_ganancia_override(rubro, subrubro, st)
    if gan_pct is None:
        gan_pct = ganancia_pct

    gan = max(0.0, min(500.0, float(gan_pct))) / 100.0
