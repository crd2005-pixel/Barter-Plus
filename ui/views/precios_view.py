...
class EtiquetasPreviewDialog(QDialog):
    ...
    def _draw_single_label(self, painter, rect, item, ppm):
        ...
        if self.chk_marca.isChecked() and item.get('marca'):
            ...
            painter.drawText(rect_txt, int(Qt.AlignmentFlag.AlignHCenter) | int(Qt.AlignmentFlag.AlignTop), txt)
            y_cursor += fm.height()
        ...
        if self.chk_nombre.isChecked() and item.get('nombre'):
            ...
            bounding = painter.boundingRect(rect_txt, int(Qt.AlignmentFlag.AlignHCenter) | int(Qt.AlignmentFlag.AlignTop) | int(Qt.TextFlag.TextWordWrap), item['nombre'])

            painter.drawText(rect_txt, int(Qt.AlignmentFlag.AlignHCenter) | int(Qt.AlignmentFlag.AlignTop) | int(Qt.TextFlag.TextWordWrap), item['nombre'])
        ...
        if extras:
            ...
            fm = painter.fontMetrics()
            rect_txt = QRectF(inner.x(), y_cursor, inner.width(), float(fm.height()))
            painter.drawText(rect_txt, int(Qt.AlignmentFlag.AlignHCenter) | int(Qt.AlignmentFlag.AlignTop), line)
        ...
        if self.chk_codigo.isChecked() and item['code']:
            ...
            if h_bars > (2 * ppm): # Draw bars if at least 2mm space
                ...
                painter.drawText(rect_txt, int(Qt.AlignmentFlag.AlignHCenter) | int(Qt.AlignmentFlag.AlignTop), item['code'])
...
from PyQt6.QtCore import QMarginsF # Import needed
...
class CodigosBarraTab(QWidget):
    ...
    def _load_data(self):
        ...
        with SessionLocal() as s:
            ...
            for r, (p, m_nombre) in enumerate(rows):
                ...
                ck.setCheckState(Qt.CheckState.Checked if not p.codigo_barras else Qt.CheckState.Unchecked)
                ...
                self.tbl.item(r, 1).setData(Qt.ItemDataRole.UserRole, p.id)
    ...
    def _get_selected_ids(self):
        ...
        for r in range(self.tbl.rowCount()):
            if self.tbl.item(r, 0).checkState() == Qt.CheckState.Checked:
